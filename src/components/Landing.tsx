...
const HORIZONTAL_PARTICLES = Array.from({ length: 20 }, (_, i) => ({
  id: `dot-h-${i}`,
  top: `${(i + 1) * 5}%`,
  duration: 5 + ((i * 7) % 10),
  delay: (i * 3) % 5,
...
const VERTICAL_PARTICLES = Array.from({ length: 20 }, (_, i) => ({
  id: `dot-v-${i}`,
  left: `${(i + 1) * 5}%`,
  duration: 6 + ((i * 11) % 9),
  delay: (i * 2.5) % 5,
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center p-6 overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none opacity-40">
        ...
        {HORIZONTAL_PARTICLES.map((particle) => (
          <motion.div
            key={particle.id}
            className="absolute h-1 w-1 bg-white/30 rounded-full"
            initial={{ left: '-10%', top: particle.top }}
            animate={{ left: '110%' }}
            transition={{
              repeat: Infinity,
        ...
        {VERTICAL_PARTICLES.map((particle) => (
          <motion.div
            key={particle.id}
            className="absolute h-1 w-1 bg-white/30 rounded-full"
            initial={{ top: '-10%', left: particle.left }}
            animate={{ top: '110%' }}
            transition={{
              repeat: Infinity,
              duration: particle.duration,
      ...
      <div className="z-20 max-w-4xl text-center flex flex-col items-center">
        ...
        >
          <button
            type="button"
            onClick={onLaunch}
            className="group relative px-8 py-4 bg-brand-green text-brand-navy font-bold text-lg rounded-sm hover:bg-brand-green/90 transition-all flex items-center justify-center gap-3 overflow-hidden shadow-[0_0_30px_rgba(0,255,136,0.3)] hover:shadow-[0_0_50px_rgba(0,255,136,0.5)] cursor-pointer"
          >
          ...
          <button
            type="button"
            className="px-8 py-4 bg-transparent border border-white/20 hover:border-white/50 text-white font-bold text-lg rounded-sm transition-all flex items-center justify-center gap-3 cursor-pointer"
          >
      ...
      <div className="absolute bottom-4 right-6 text-xs text-white/30 font-mono text-right z-20 pointer-events-none">
        AI Emergency Traffic Corridor System â€¢ Open-source prototype<br/>
